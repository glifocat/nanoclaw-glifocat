 ...
 * column).
 *
 * Note: migration 007 (pending-approvals-title-options) already handles the
 * retroactive edit to `pending_approvals`. This migration covers everything
 * else that 007 missed: `pending_questions` columns and the entire user-level
 * privilege model tables.
 *
 ...
 * `messaging_groups.admin_user_id`, `agent_groups.container_config`) are not
 * reversed — there are no readers of those columns in current code, and
 * SQLite does not support `DROP COLUMN` without a table rewrite, so they are
 * left as harmless dead columns.
 */
